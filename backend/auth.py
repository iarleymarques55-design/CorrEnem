...
def _enviar_email_verificacao(email_destino: str, nome: str, codigo: str) -> bool:
    ...
    # 1. Tentativa via SendGrid HTTP API (Porta 443 — provedor consolidado, costuma ter melhor reputação de IP)
    if sendgrid_api_key:
        try:
            ...
            payload = {
                "personalizations": [{"to": [{"email": email_destino, "name": nome}]}],
                "from": {"email": email_remetente or "naoresponda@correnem.com", "name": "CORRENEM"},
                "subject": "Confirme seu e-mail - CORRENEM",
            ...
            print(f"[AUTH] Erro via SendGrid API: {sendgrid_err}")

    # 2. Tentativa via EmailJS REST API (Porta 443 — chamado pelo backend, código nunca vai ao navegador)
    ...
    # 3. Tentativa via Resend HTTP API (Porta 443 — 100% liberada no Railway)
    if resend_api_key:
        try:
    ...
    # 4. Tentativa via Brevo HTTP API (Porta 443 — 100% liberada no Railway)
    if brevo_api_key:
        try:
...
async def solicitar_reset(dados: SolicitarResetRequest, request: Request, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "mensagem": (
            ...
            if email_enviado
            else "Não foi possível enviar o e-mail agora. Tente novamente em instantes."
        ),
        "email_enviado": email_enviado,
        "codigo_console": codigo if (not email_enviado and SHOW_CONSOLE_CODES) else None
    }
