...
- Envio de e-mail via Brevo HTTP API (Porta 443 — compatível com Railway)
"""
import os
import json
import string
...
BREVO_REMETENTE_EMAIL = os.getenv("BREVO_REMETENTE_EMAIL", os.getenv("EMAIL_REMETENTE", "")).strip()
BREVO_REMETENTE_NOME  = os.getenv("BREVO_REMETENTE_NOME", "CorrEnem").strip()
CODIGO_EXPIRA_MINUTOS = 10

# Exibe códigos de console apenas em desenvolvimento
SHOW_CONSOLE_CODES = (
    os.getenv("ENVIRONMENT", "production").lower() in ("development", "dev", "local", "debug")
    or os.getenv("SHOW_CONSOLE_CODES", "false").lower() == "true"
)
...
# ── Envio de E-mail via Brevo HTTP API (Porta 443) ───────────────────────────

def _enviar_email_verificacao(email_destino: str, nome: str, codigo: str) -> bool:
    """
    Envia o e-mail de verificação via Brevo HTTP API (Porta 443).
    Funciona 100% no Railway sem bloqueios de porta SMTP.
    Retorna True se enviou com sucesso, False caso contrário.
    """
    if not BREVO_API_KEY:
        if SHOW_CONSOLE_CODES:
            print(f"[AUTH-DEV] BREVO_API_KEY não configurada. Código para {email_destino}: {codigo}")
        return False
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            if resp.status in (200, 201):
                print(f"[AUTH] E-mail enviado com sucesso via Brevo para {email_destino}")
                return True
            print(f"[AUTH] Brevo retornou status inesperado {resp.status}")
            return False
        ...
        print(f"[AUTH] Erro ao enviar e-mail via Brevo: {err}")
        return False
...
async def cadastrar(dados: CadastroRequest, request: Request, db: Session = Depends(get_db)):
    """
    Registra um novo usuário no PostgreSQL e envia código de verificação por e-mail.
    """
    ...
    if usuario_existente:
        if usuario_existente.verificado:
            # Conta Google sem senha: permite associar senha
            if not usuario_existente.senha_hash and usuario_existente.provedor == "google":
                ...
                if dados.telefone and dados.telefone.strip() and not usuario_existente.telefone:
                ...
                return {
                    "sucesso": True,
                    "mensagem": "Senha associada com sucesso à sua conta Google! Agora você pode entrar com e-mail/senha ou com o Google.",
                    "usuario": {
            ...
            # Conta já existe com senha: bloqueia
            raise HTTPException(
                status_code=409,
                detail="Este e-mail já está cadastrado. Faça login ou use 'Esqueci minha senha' para recuperar o acesso."
            )
        else:
            # Usuário cadastrado mas ainda não verificado: reenviar código
            ...
            # Atualiza hash da senha no usuário existente
            usuario_existente.senha_hash = _hash_senha(dados.senha)
            db.commit()
            ...
            return {
                "sucesso": True,
                "mensagem": "Código de verificação reenviado. Verifique sua caixa de entrada.",
                "email_enviado": email_enviado,
                "codigo_console": codigo if (not email_enviado and SHOW_CONSOLE_CODES) else None
            }

    # Novo usuário: cria conta não-verificada e envia código
    novo_usuario = User(
        nome=dados.nome.strip(),
        email=email_lower,
        senha_hash=_hash_senha(dados.senha),
    ...
    return {
        "sucesso": True,
        "mensagem": "Conta criada! Verifique sua caixa de entrada e insira o código de 6 dígitos para ativar sua conta.",
        "email_enviado": email_enviado,
        "codigo_console": codigo if (not email_enviado and SHOW_CONSOLE_CODES) else None
    }
...
async def login(dados: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    # Migração automática de hash legado SHA-256 → PBKDF2-HMAC-SHA256
    if not usuario.senha_hash.startswith("pbkdf2$"):
...
async def login_google(dados: GoogleLoginRequest, db: Session = Depends(get_db)):
    """
    Autentica ou cadastra um usuário via Google OAuth no PostgreSQL.
    - Se o e-mail já existe com senha local: bloqueia com mensagem clara.
    - Se o e-mail já existe sem senha (conta Google pura): atualiza e loga.
    - Se não existe: cria conta Google verificada automaticamente.
    """
    email_lower = dados.email.strip().lower()
    usuario = db.query(User).filter(User.email == email_lower).first()

    if usuario:
        # Conta local com senha: bloqueia login Google para evitar duplicata
        if usuario.senha_hash and usuario.provedor != "google":
            raise HTTPException(
                status_code=409,
                detail=(
                    "Este e-mail já está cadastrado com senha. "
                    "Faça login com e-mail e senha, ou use 'Esqueci minha senha' para recuperar o acesso. "
                    "Após entrar, você pode vincular o Google nas configurações de perfil."
                )
            )
        # Conta Google ou sem senha: atualiza e loga
        usuario.verificado = True
...
async def solicitar_reset(dados: SolicitarResetRequest, request: Request, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "mensagem": (
            ...
            if email_enviado
            else "Use o código exibido abaixo para redefinir sua senha."
        ),
        "email_enviado": email_enviado,
        "codigo_console": codigo if (not email_enviado and SHOW_CONSOLE_CODES) else None
    }
...
async def atualizar_perfil(dados: AtualizarPerfilRequest, db: Session = Depends(get_db)):
    ...
    Identificado pelo e-mail. Campos ausentes (None) são ignorados.
    """
    email_lower = dados.email.strip().lower()
    usuario = db.query(User).filter(User.email == email_lower).first()
